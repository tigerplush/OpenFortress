use bevy::prelude::*;
use common::types::{ChunkBlockCoordinates, ChunkCoordinates, WorldCoordinates};
...
impl Chunk {
    ...
    pub(crate) fn remove_block(&mut self, block_coordinates: ChunkBlockCoordinates) {
...
impl ToChunkAndBlock for WorldCoordinates {
    fn to_chunk_and_block(&self) -> (ChunkCoordinates, ChunkBlockCoordinates) {
        (
            ...
            ChunkBlockCoordinates(self.0.rem_euclid(CHUNK_SIZE.as_ivec3()).as_uvec3()),
        )
    }
}

/// returns the index of a tile in it's block array by coordinates
pub(crate) fn to_index(coordinates: impl Into<ChunkBlockCoordinates>) -> usize {
    let block_coordinates: ChunkBlockCoordinates = coordinates.into();
